...
**Should we add `Route::resource()` for RESTful routes?**  
❌ No. It hides 7 routes behind one declaration. Explicit is better than compact.

**Should we auto-inject repositories by convention?**  
❌ No. Type-hint the repository in the constructor. It's one line.

**Should we add a query builder?**  
⚠️ Maybe, but it must:
- Not hide SQL (you should see the query being built)
- Not pretend to be database-agnostic
- Be clearly labeled as "convenience, not requirement"

**Should we add `#[Cache]` attribute for method caching?**  
✅ Yes! It's explicit, visible, and doesn't hide behavior.
...
1. **I will not add convenience at the cost of explicitness.**
2. **I will not hide SQL behind abstractions.**
3. **I will not introduce global state.**
4. **I will not copy Laravel patterns without questioning them.**
5. **I will prioritize understanding over speed.**

This framework is not for everyone. It's for teams that value **clarity** over **magic**.
